    ...
    <div className="glass p-6 rounded-2xl border-black glow-accent">
      <div className="flex items-center gap-6">
        <button 
          ...
          style={{ background: 'var(--color-accent)', color: '#0066FF' }}
        >
          {playing ? (
          ...
          <div className="absolute top-1/2 left-0 right-0 h-4 -translate-y-1/2 pointer-events-none z-10 w-full">
            {markers.map((marker, idx) => {
              ...
              return (
                <div 
                  key={marker.id || idx}
                  className="absolute top-1/2 -translate-y-1/2 -ml-1.5 w-3 h-3 rounded-full bg-white border-2 border-[#0066FF] pointer-events-auto cursor-pointer shadow-sm hover:scale-150 transition-transform origin-center"
                ...
                >
                  ...
                  {hoveredMarker === marker && (
                    <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-white text-[#0066FF] text-[10px] px-2 py-1 rounded shadow-lg whitespace-nowrap z-50 pointer-events-none">
