...
const STATUS_META = {
  ...
  review:      { label: 'Review',     color: '#ffffff',      bg: '#000000'      },
  revision_requested:      { label: 'Revision',     color: '#ffffff',      bg: '#000000'      },
  finalized:      { label: 'Final',     color: '#ffffff',      bg: '#000000'      },
};
  ...
  return (
    <motion.div
      ...
      className={`group flex items-center gap-4 px-4 py-3 cursor-pointer transition-all border-b border-gray-50 last:border-0 ${
        isSelected ? 'bg-black text-white' : 'hover:bg-gray-50 text-black'
      }`}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className={`text-sm font-black uppercase truncate ${isSelected ? 'text-white' : 'text-black'}`}>
          {order.projectName || 'Untitled Order'}
        </div>
        <div className={`text-[10px] flex gap-2 opacity-50 font-bold uppercase ${isSelected ? 'text-white' : 'text-black'}`}>
...
export default function ProTrackSelector({ orders, loading, selectedOrder, onSelect }) {
  ...
  return (
    <div className="border-2 border-black bg-white shadow-[8px_8px_0px_#000] overflow-hidden">
      {/* ── Header ── */}
      <header className="px-5 py-4 bg-black text-white flex items-center justify-between border-b-2 border-black">
        <div className="flex items-center gap-3">
          <Music2 size={16} />
          <h3 className="text-[10px] font-black uppercase tracking-[0.2em]">Live Sessions</h3>
        </div>
        <div className="flex items-center gap-4">
          <span className="text-[9px] font-black uppercase opacity-50 px-2 py-1 border border-white/20">
          ...
          <div className="relative group">
             ...
             <input 
               ...
               className="bg-white/10 hover:bg-white/20 focus:bg-white text-xs text-white focus:text-black px-10 py-2 border border-white/20 focus:border-white transition-all outline-none uppercase font-bold placeholder:text-white/40"
             />
             {query && (
               <button onClick={() => setQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/40 hover:text-white">✕</button>
             )}
          </div>
        </div>
      </header>
      ...
      <AnimatePresence>
        {previewId && (
          <motion.div
            ...
            animate={{ height: 'auto' }}
            exit={{ height: 0 }}
            className="bg-black text-white px-5 py-3 flex items-center justify-between border-t-2 border-black border-opacity-20"
          >
