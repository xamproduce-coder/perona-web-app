...
const logoImg = '/logo.png'; // Reference to public/logo.png
...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[1000] flex justify-center pt-6 pointer-events-none">
      <nav className="pointer-events-auto relative flex items-center gap-3 pr-3 pl-12 py-2 rounded-full bg-slate-50/80 backdrop-blur-2xl border border-[#0066FF]/10 shadow-[0_16px_40px_rgba(0,0,0,0.8)] transition-all duration-500 hover:bg-white/90">
        ...
        <button
          onClick={scrollToTop}
          className="p-2.5 rounded-full text-[#0066FF]/50 hover:text-[#0066FF] hover:bg-white/10 transition-all duration-300 active:scale-90"
          aria-label="Home"
          title="Scroll to Top"
        >
          <Home size={18} />
        ...
        <button
          ...
          className="
            relative w-10 h-10 rounded-full bg-gradient-to-tr from-[#1D4ED8] to-[#3B82F6]
            flex items-center justify-center overflow-hidden group
            hover:scale-[1.12] active:scale-95 transition-all duration-300
            shadow-[0_0_20px_rgba(29, 78, 216,0.5)] hover:shadow-[0_0_30px_rgba(29, 78, 216,0.7)]
          "
          aria-label="Dashboard"
          title="My Dashboard"
        >
          {/* Inner dark circle to make EQ bars pop */}
          <div className="absolute inset-0 bg-white/10 rounded-full group-hover:bg-transparent transition-colors duration-300" />
        ...
        <button
          ...
          className="
            px-6 py-2.5 rounded-full bg-gradient-to-r from-[#1D4ED8] to-[#3B82F6] text-[#0066FF]
            text-xs font-black uppercase tracking-[0.15em]
            hover:scale-[1.05] hover:shadow-[0_10px_30px_rgba(29, 78, 216,0.5)]
        ...
        {/* RECORD WITH ME — SSL Style */}
        <button
          onClick={() => navigate('/record')}
          className="btn-ssl !py-2 !px-4 !text-[11px]"
        >
